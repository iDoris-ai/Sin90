...
## M5+ — 用户拍板追加（2026-09-24，Q3/Q4/Q6/Q7/Q8）

### T5.2.2 classify 兜底到「待定」  `BACKLOG`
- **优先级**：mid
- **目标**：没有合适的 Direction 时，把条目归到一个固定的「待定」分类，不提议新建 Direction（Q4）。
- **开发范围**：先在 DESIGN §2 定义「待定」的数据形态（系统保留的 Direction，或 inbox 上的一个标记，二选一，先写设计）；classify 兜底产出 `AssignTaskDirection(t, 待定)`；「待定」不参与配额和缺口计算。
- **依赖**：T5.2.1
- **验收命令**：`cargo test classify_fallback_`：无匹配 → 归到待定；正对照：有匹配 → 归到真实 Direction；「待定」不出现在 propose 的缺口里。
- **证据**：

### T5.2.3 classify 低置信度不出提议  `BACKLOG`
- **优先级**：mid
- **目标**：模型步的置信度低于阈值时，结果记 `nothing`，不提交提议（Q8）。阈值做成配置，默认值由 T5.5.1 的冒烟数据确定。
- **依赖**：T5.2.1
- **验收命令**：`cargo test classify_low_confidence_`：低于阈值 → 无提议、调用行照记；正对照：高于阈值 → 出提议。
- **证据**：

### T5.6.1 AI 自动触发设计  `BACKLOG`
- **优先级**：mid
- **目标**：AI 在需要时自动运行（Q3）：用户动作（如记下新条目、周进入 planning）、定时周期（借内核调度回调 `_a24/scheduler/*`）、关键词。先写设计文档，冻结后再实现。
- **开发范围**：`docs/design/` 新文档：触发源清单、去抖 / 限频、与单飞槽位和去重的关系、关闭开关（默认开还是关由设计提出、用户确认）。
- **依赖**：T5.5.1、Agent24 ME4-1.5.1（定时触发依赖调度回调）
- **验收命令**：设计文档经对抗评审冻结。
- **证据**：

### T5.6.2 AI 自动触发实现  `BACKLOG`
- **优先级**：mid
- **依赖**：T5.6.1
- **验收命令**：按 T5.6.1 冻结的判据。
- **证据**：

### T5.7.1 拒绝提议 + 拒绝日志  `BACKLOG`
- **优先级**：high（没有拒绝入口，挂起提议会越积越多，还会挡住去重）
- **目标**：`POST /proposals/{id}/reject`（只人类 key）；提议状态变为 `rejected`，从待处理列表消失，不再参与去重（Q6）。
- **开发范围**：**拒绝必须留日志**，为将来学习用户习惯积累数据：记下提议 id、能力（classify/summarize/propose/人工）、ops 摘要、AI 给的理由、创建时间、拒绝时间、可选的拒绝原因（请求体可选字段 `reason`）；同时发 `proposal.rejected` 事件。日志表结构先进 DESIGN §2。
- **依赖**：T5.2.1（去重逻辑要同步认 rejected）
