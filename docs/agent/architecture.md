...
5. **回调通道多路复用**（T3.2.0 起）：并发调用不互相阻塞；内核客户端不依附于 EventSink。
   **回调连接断了 = 这一代结束**（Agent24 内核合约：一代只服务一条连接，第一次 accept 后监听 socket 即关闭删除，同一代不许重连 —— `agent24-os-proto/src/endpoint.rs`、`supervisor.rs`）。
   Sin90 一旦判定回调连接死亡（EOF、写失败、收到非法/超长帧），就关闭连接并以非零码退出进程，由内核 supervisor 按退避重启；**不做重连**。
   在途调用得到「结果不确定」错误，**不自动重试**（可能已被内核执行）；是否重发由调用方按幂等性决定（outbox 天然幂等）。
