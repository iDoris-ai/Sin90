...
按「现在能定的」和「握手细节」分开。混着写，等于给自己安排返工。
...
`src/adapter_agent24/mod.rs` 已经实现了真实的 `initialize` 握手与事件上报（`_a24/events/emit`），不再是桩。`domain-os.yml` 也已经是 `impl_kind: out_of_process_provider`。

已经落地、写代码时可以直接依赖的两条约束：

- **单帧上限 1 MiB，超限断连、不降级。** **别把大 payload 塞进单帧。**
- **版本区间协商，不报区间 = 不兼容。** 沉默不会被当成同意。`initialize` 必须报 `[min, max]`（见 `PROTOCOL_MIN`/`PROTOCOL_MAX`）。
