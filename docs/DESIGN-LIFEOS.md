...
| 24 | M5 用内核私有记忆（T4.4.1 写入的定稿摘要）做 AI 上下文 | **v1 不用** | SQLite 是真相（architecture.md 边界 #1），summarize 需要的「上周定稿」本地直接可读；走 `_a24/memory/private/recall` 只多一个失败面，且 v1 三个能力没有一个需要跨设备/跨模块上下文。T4.4.1 照做（它的价值是给内核侧 agent 用）。**复审触发条件**：出现需要「非 Sin90 数据」的 AI 能力。**T4.4.1 store 层实现（2026-09-26，`feat/t4.4.1a-review-remember-store`）**：`finalize_review`（`store::repo`）在把 Review 从 `draft` 翻到 `finalized` 的**同一个事务**里，调用 `upsert_outbox` 入队一行 `kind = "memory.remember"`、`dedup_key = "review:<id>"`；`desired` 是 `review_remember_desired` 产出的派生摘要 `{dedup_key, review_id, review_kind, period, summary, finalized_at}`——`summary` 是 `current.body`（定稿前读到的正文）截到 `REVIEW_SUMMARY_MAX_CHARS = 500` **字符**（`.chars()` 计数，不按字节，CJK 文本按字符数计费，超出加 `…`）。`body.trim()` 为空（最常见：从未 `PATCH` 过的默认空草稿）时整个跳过入队——没有值得摘要的内容。`finalize_review` 是 `draft -> finalized` 的单向门（`core::transitions::review_transition_allowed` 只有这一条边，无撤回路径），所以这个 `dedup_key` 这辈子只会入队一次。真正把这行送到内核、并处理"`_a24/memory/private/remember` 本身不幂等"的问题，在对账器那一层（`feat/t4.4.1-final-to-memory`，见下一条 T4.4.1 记录）。**T4.4.1 对账器实现 + Opus 评审整改（2026-09-26，`feat/t4.4.1-final-to-memory`）**：`adapter_agent24::reconciler::apply_one` 把 `memory.remember` 接成对账器的第三种 outbox kind，复用 T3.3.2 的 version CAS / 重读 / 错误分类 / 泵。`_a24/memory/private/remember` 本身不幂等（内核每次成功调用都铸造新 id，没有按 key 去重这回事）——`remember_review_summary` 在真正调用 `remember` 之前，先用这行自己携带的 `dedup_key` 去 `_a24/memory/private/recall` 查一遍（`page_size` 用内核允许的上限 50，按 `cursor` 翻页最多 10 页——内核 `recall` 的真实语义是子串匹配、新到旧扫描、每次最多扫 2000 行，`cursor` 非空只代表"这次扫描窗口用完了"不代表"没有更多匹配"，翻完 10 页仍未命中判"未知"，走现有的退避重试而不是当"确实不存在"处理，接受的代价是积压时可能因此吃到内核的 `RateLimited` 顶回来），命中就直接当已完成处理，不再第二次调用 `remember`——不开新表，靠 body 里的 `dedup_key` 让内核自己的 `recall` 当查重的事实来源。客户端收进 `ReconcilerClients { scheduler: Option<SchedulerClient>, memory: Option<MemoryClient> }`（两者独立可缺，`SchedulerClient`/`MemoryClient` 都补了 `Clone`）：`main.rs` 只要有一个被授予就起泵，`reconcile_full` 在 `scheduler` 缺时直接 no-op；某个能力这一代没被授予时，对应 kind 的行完全不碰、留 `pending`（不再标 `failed(forbidden)`——`failed` 是终态，`outbox_due_pending` 再也捞不到，会永久丢掉下一代本可以补上的行；Opus 评审 H1 纠正的正是这一点）。`sin90_outbox.result_ref`（迁移 `0013_outbox_result_ref.sql`，见本表 `sin90_outbox` 行）在这一层真正落地：成功 `remember`（或 recall 命中已有记忆）拿到的 id 写进这一列。如果未来出现撤回再定稿：今天按 `dedup_key` 精确相等的查重会把"内容变了的重新定稿"误判成"已经记过"而漏写新摘要，需要把内容指纹（如 `summary`/`body` 的 hash）揉进 `dedup_key` 一起判断——这是留给那条未来路径的设计债，本轮不实现。 |
...
| 29 | 对账器（T3.3.2）「内核侧 `user_suspended`」的可见性怎么存 | **`sin90_routines` 加列 `kernel_suspended_at TEXT NULL`，不建独立表** | Opus 评审 M1：最初实现建了一张独立表 `sin90_routine_kernel_notices`（`routine_id PK, note, updated_at`），仿 #28 `sin90_proposal_rejections` 的"独立表"理由——但那条理由成立的前提是"这是一份只追加的历史日志"；这里恰恰相反，它是**内核当前状态的一个布尔快照**，每一轮全量对账都要整体重算（先把所有行清空，再按内核这次 `list` 的回答重新置位），从未打算保留历史。一张"每次都整表重建"的表，语义上就是 `sin90_routines` 自己的一列，硬拆成表反而要多维护一次 join、一次外键、一次"孤儿行"清理（Routine 被彻底删除时——虽然今天没有硬删除路径）。定为 `TEXT NULL`（ISO-8601 时间戳，`NULL` = 当前未被暂停）而不是 `BOOLEAN`，是为了让"最后一次观察到暂停"这个时间信息免费带上，不需要额外一列。写这一列**不算**一次 Routine 变更：不 bump `updated_at`、不写 `sin90_events` 行（它是对账器对内核状态的观测，不是人或 AI 对 Routine 实体做的决定）。`/today` 的 `kernel_suspended_routines` 段读 `kernel_suspended_at IS NOT NULL AND status <> 'retired'`（后一个条件是防御性的——正常情况下 `retired` 的 Routine 从不会被对账器重新置位，因为它已经从"本地期望状态"集合里被排除，见 §2 #15/#16 的对账器落地条目）。迁移 `0011_outbox_version_kernel_suspended.sql`（与下一条 C1 的 `sin90_outbox.version` 同一个文件，两者都是本轮对账器评审一起浮现的加列需求）。 |
| 30 | classify 兜底「待定」的数据形态（tasks.md T5.2.2，用户拍板 Q4）：**A. 系统保留的 Direction** vs **B. inbox 上的一个标记** | **A：系统保留的 Direction，固定 id `sin90-triage`，标题「待定」，`area_id = NULL`，`status = active`** | **Op 本身已经决定了答案**：tasks.md T5.2.2 的开发范围原文是"classify 兜底时产出 `AssignTaskDirection(t, 待定)`"——`AssignTaskDirection{task_id, direction_id}`（§11.2.1）只能指向一个 Direction，没有第二个参数能表达"标记 inbox 条目"；选 B 就要么破坏这个 Op 的签名，要么绕开它走一条新的直写路径，两者都违反"AI 只能提议"（§10.1）。A 是唯一不需要新 Op、不需要新校验分支就能落地的选项。<br><br>**兜底规则本身（2026-09-26 round 2，Opus 评审 REQUEST_CHANGES 后统筹拍板收窄）**：Q4「没有合适 Direction 时归待定」不能读成"只要 classify 这一轮没给出真实 Direction 就归待定"——那会把"模型根本没来得及看"（离线/熔断/standalone 无模型）、"这台机器还没配置任何 Direction"（新用户）也悄悄算成"经过判断、确实不合适"，越权代答。收窄为**只有一种情况**归待定：**某个模型步骤真的跑完了一次调用，并且以 `medium` 或以上把握明确回答 `"none"`**（`ClassifyDecision.direction_id == None && !low_confidence`，且这个 `None` 必须来自 `Outcome::Produced`，不是 `Outcome::Nothing`）。其余「没结果」形状一律回到裸 `nothing`，`ClassifyItem.reason` 记新增的 `"undetermined"` 标签（`"low_confidence"` 仍单独存在，两者都表示"这轮没有答案"但成因不同），留给下一次触发重试：<br>&nbsp;&nbsp;- **候选集为空**（新用户一个 Direction 都没有）→ `nothing/undetermined`，不写调用行、不产提议——这是 T5.2.2 最初设计里"候选为空"的原始行为，只是加了 `reason` 标签。<br>&nbsp;&nbsp;- **整条引擎梯耗尽**（`Outcome::Nothing`：R1 undecided、R2 no_match、模型不可达或全部降级——离线、熔断、standalone 无模型配置）→ `nothing/undetermined`，不产生提议；`classify_remote_down_local_up`（J16）自己的正对照就是这个场景，已改回不产生提议。<br>&nbsp;&nbsp;- **模型明确回答 `"none"` 但 `confidence: "low"`**（Q8 本意）→ `nothing/low_confidence`——`parse_classify_reply`（`src/ai/classify.rs`）的 `"none"` 分支以前无条件写死 `low_confidence: false`，是本轮修的一个真实 bug（H1）：一个"没把握"的 `"none"` 和一个"有把握"的 `"none"` 以前落到同一个待定提议里，分不清楚。<br><br>**逐点对比**：<br>① **对现有 Op/validate 的影响**：A 天然满足 A4/A5（Direction 存在、非终态）——待定被迁移 `0014_triage_direction.sql` 播种为 `status = active`，`AssignTaskDirection`**零改动**就能把它当目标；B 需要给 `AssignTaskDirection` 或一条新 Op 加"标记 inbox 条目"的语义，且要重新定义 A3（"仍在 inbox"）在有标记时是否还成立，动的是校验核心逻辑而不是新增一行数据。<br>② **用户后续怎么把「待定」里的条目重新归类**（followup，不在本次范围内实现）：一个任务被指派到待定后，**在 `direction_id IS NULL` 这个 inbox 的既有定义下就离开了 inbox**。**前提**：H2（下方④）已经把 `title_history` 里的待定行为排除掉——如果这条前提不成立，放宽 inbox 定义只会让"待定→再看一遍→R1 从历史一眼又判回待定"变成死循环，比现在的"永远出不来"更糟。H2 已经修了，所以这条 followup 现在是安全的：把 `AiReadModel::inbox`/`inbox_task`（`src/store/ai_port.rs`）的判定从 `direction_id IS NULL` 放宽成 `direction_id IS NULL OR direction_id = 'sin90-triage'`，`AssignTaskDirection` 的 A3（`NotInInbox`）同样放宽；**不放宽"待定→待定"这个方向**——重新分类只有在出现新的真实 Direction、模型这次给出真实候选键时才应该发生，不该对着候选集里没有它自己（待定已被 direction_candidates 排除）的情况反复空转。**复审触发条件**：用户反馈"待定里的任务永远出不来"，再落地这条 followup 并补测试。<br>③ **不能被用户误删/改名**：A 用固定 id 保护（不用 `is_system` 列）——今天 `src/http/mod.rs` 只有 `POST /directions` + `GET /directions`,没有任何路由能改名或删除一个 Direction,所以"固定 id"已经是全部的保护面;未来一旦出现改名/删除路由,那条路由自己按这个 id 特判即可,不需要现在就加一列没人读的标记。<br>④ **对 R1/周复盘/配额/缺口计算/`/today` 的影响**：**不被 R1 当候选** 靠 `AiReadModel::title_history`（`src/store/ai_port.rs`）实现（H2，2026-09-26 round 2 补，之前是遗漏）——一个待定分类过的任务在 `direction_id IS NOT NULL` 的字面意义上"已归类"，如果不排除，R1 会把"上一条同标题任务被归了待定"当成决定性历史证据，让**之后每一条同标题任务都被 R1 直接判给待定，模型再也不会被问一次**，即使用户已经建了完美匹配的新 Direction——`title_history` 的 SQL 因此加 `AND d.id != ?`（绑定 `TRIAGE_DIRECTION_ID`）。**不被 R2/模型当候选** 靠 `AiReadModel::direction_candidates` 排除（`AND d.id != ?`，同一个 id）。`gap_directions`（`src/ai/propose.rs`）同样显式排除 `sin90-triage`——待定不会被算进某周的"缺口 Direction"，即使一条 Rhythm 分配（`AdjustRhythm`）手动把配额分给了它（`check_alloc` 今天不拒绝这个 id，见下方"残余风险"）；复盘（`ai::summarize`/`weekly_draft`）不特殊处理它——因为它是一个普通、非终态、有标题的 Direction，会像任何其它 Direction 一样在草稿里单独占一行统计，这正是 tasks.md 原文"复盘里单列一行"的要求，不需要额外代码。<br><br>**残余风险（接受，写明谁来盯）**：`AdjustRhythm`/`check_alloc`（`src/core/proposal.rs`）今天不拒绝一条名字是 `sin90-triage` 的分配——用户理论上可以手动给待定分配配额，但它永远不会在 `gap_directions` 里显现为缺口，所以这个分配是被接受但功能上无效的（不会被 propose.create 用到）。当前判定：功能上无害，暂不加校验；若用户反馈"分了配额却从来没被用到，很困惑"，再在 `check_alloc` 里加一条拒绝规则（新 `ProposalError` 变体）。 |
| 31 | 被拒建议条件压制 + 「待定」重新分类的数据来源：**建议指纹要不要单独存一列** | **不新增列，JOIN 回 `sin90_proposals.ops`** | §2 #28 自己的前提就是答案：`sin90_proposals` 的行永远不删、`ops` 永远不改写——`sin90_proposal_rejections.proposal_id` 已经是外键，`JOIN sin90_proposals ON id = proposal_id` 拿到的 `ops` 保证和拒绝那一刻完全一致。再单独存一份"指纹"只是把同一份数据复制到第二处，且 `Sin90Op` 未来长新变体时两处都要改，纯增加漂移面——跟 #28 自己"不给 `sin90_proposals` 加列、宁可查 `sin90_ai_calls`"的理由是同一个理由。新增 `Sin90Store::list_rejected_ops(capability_source)`（`repo.rs`），一次 JOIN 拿到 `{ops, rejected_at}` 列表，供下面每条压制逻辑自己按需要的字段匹配——不引入迁移。<br><br>**指纹定义**（能力 + 目标 + 规范化 ops，逐能力落地，而不是对 `ops` 做字节级 `==`）：<br>&nbsp;&nbsp;- classify `AssignTaskDirection{task_id, direction_id}`：指纹 = `(task_id, direction_id)`——但**压制发生在任务被选入 classify 候选之前**（`http::ai_classify::dedup_targets`），这一步还不知道这一轮会推荐哪个 `direction_id`，所以实际生效粒度收窄成 `task_id`——只要这个任务在"情况没变"期间还有任何一条被拒的 `AssignTaskDirection`，就不再把它选入候选（等价于"同一任务在情况不变时只会被推荐同一个结果"这条假设：R1/R2/model 都是纯函数于当前状态，状态不变，结论不变，见下面"情况变化"判定）。<br>&nbsp;&nbsp;- propose `CarryOverTask{task_id, to_week}`：指纹 = `(task_id, to_week)`，与 `dedup_propose` 现有的 pending 去重粒度（`excluded_carry_task_ids`）完全一致，直接把 REJECTED 的匹配项并入同一个 `HashSet`。<br>&nbsp;&nbsp;- propose `ReorderTasks{week_id, order}`：指纹 = `(week_id, order)`——`order` 保留原始顺序，不排序、不去重比较；复用 pending 去重已有的"`order` 恰好覆盖当前非终态任务集合"这条"仍然有效"判定（`order_set == current_non_terminal_ids`），只是判定源从"某条 PENDING 提议"换成"某条 REJECTED 提议"。<br>&nbsp;&nbsp;- propose `CreateTasks{week_id, tasks}`：指纹收窄到 `(week_id, direction_id)`——与 classify 同一个理由：压制发生在"这一轮会不会再对着这个缺口 Direction 生成新任务"这一步，这一步还不知道模型这次会写出什么标题，没有"逐字比较标题"的立足点；复用 `dedup_propose` 现有的 per-direction 排除（`excluded_create_direction_ids`）粒度——一条被拒的 `CreateTasks{week_id, tasks: [...]}` 按 `tasks[].direction_id` 拆成多个 `(week_id, direction_id)` 指纹，与 PENDING 那一半共享同一个 `HashSet`。<br>&nbsp;&nbsp;- summarize `DraftReviewBody{review_id, base_body_sha256, body}`：指纹 = **只有 `review_id`，`body`/`base_body_sha256` 不参与**。理由：summarize 每次运行都会让模型重新生成正文措辞，即使"这周该不该重写"这件事完全没变，逐字比较 `body` 几乎永远判不相等，压制形同虚设；`DraftReviewBody` 本身是整篇替换（§11.2.2），"针对这个 review 的重写建议被拒绝了"这个语义天然只挂在 `review_id` 上，与措辞无关——同一理由 `http::ai_summarize::dedup_summarize` 对 PENDING 的判定也早就是"任何一条仍有效的 `DraftReviewBody`"而不比较 body。<br><br>**情况变化判定**（OR 条件，逐能力落地，不是硬套同一份代码）：<br>&nbsp;&nbsp;- classify：`task.updated_at > rejected_at`（真正的"任务被修改过"）**或** 存在一个 `created_at > rejected_at` 的非终态、非待定 Direction。第一腿必须只在**任务此刻仍是 `direction_id IS NULL`**（纯 inbox，从未被任何 `AssignTaskDirection` 接受过）时才采信——`AssignTaskDirection` 的 apply（`repo.rs` 的 CAS `UPDATE ... SET direction_id = ?, updated_at = ? WHERE id = ? AND direction_id IS NULL`）本身也会 bump `updated_at`，这不是"用户修改了任务"，是"AI 的另一条提议被接受了"；而这条 UPDATE 的 `WHERE` 子句保证一旦命中，`direction_id` 必然从 NULL 变成非 NULL——换句话说，只要此刻 `direction_id` 依然是 NULL，`updated_at` 就不可能被"接受一个 AssignTaskDirection 提议"这件事碰过，唯一还能碰它的路径是 `transition_task`（人类直写的状态流转，`PATCH /tasks/{id}`）。这就是为什么这条判定天然只对纯 inbox 任务生效、不对已经落到「待定」的任务生效——落到「待定」的任务走的是下面④「待定重新分类」自己的判据，不查这张表。第二腿复用 `AiReader::max_eligible_direction_created_at()`（新方法，`store/ai_port.rs`，逻辑与 `direction_candidates`/`title_history` 共享的"非终态 + 排除 `TRIAGE_DIRECTION_ID`"口径完全一致）。<br>&nbsp;&nbsp;- propose：只用"新 Direction"这一条腿，`task 被修改过`这条腿本轮不接——reorder/create 没有单一"目标任务"可比较 `updated_at`（create 的目标是尚不存在的新任务；reorder 的目标是整周任务集合，"变没变"已经由复用的 order-covering 检查天然覆盖：任务集合一变，`order_set != current_non_terminal_ids`，判定自动失效，不需要额外代码）；carry 虽有单一目标任务，但为了三个子建议共享同一套判定代码、控制本轮改动面，統一只接"新 Direction"腿——**复审触发条件**：真实场景里出现"同一个 carry 建议被拒后，用户改了那个任务的状态，但因为没有新 Direction 出现所以一直重复不了"的反馈，再给 carry 单独补第一腿。<br>&nbsp;&nbsp;- summarize：`live_body_sha256(review.body) != base_body_sha256`（复用 D6 `StaleBase` 已有的同一份 SHA-256 比较——被拒那条 `DraftReviewBody` 自己的 `base_body_sha256` 就是"提议生成时看到的正文摘要"，与当前正文的摘要一比较，天然就是"人在这之后又编辑过正文"）。"新 Direction" 腿对 summarize 不适用——复盘正文本来就统计"当周全部 Direction"，多一个 Direction 不会让"这周该不该重写"这件事本身产生新证据，留空不是遗漏，是判定过范围后的显式排除。<br><br>**run 结果里的 `skipped` 原因**：`AiRunItem.reason` 新增静态串 `"suppressed_rejected"`，与既有的 `"dedup"`（挂起提议压制）区分——`http::ai_classify::dedup_targets`/`build_classify_run_items` 与 `http::ai_summarize::dedup_summarize` 都能在条目粒度分辨两者，各自返回一个"why"标签而不是裸 `bool`；propose 的 `ProposeItemResult::Skipped` 本身就是"整个 kind 一次没试"的粗粒度信号（reorder/create 各一个 `bool` 短路），REJECTED 来源的排除只是并入了 `excluded_*`/`skip_*` 这些既有集合，不会单独产生一个"只因为拒绝才 Skipped"的 wire 值——**已知的、接受的范围收窄**：propose 侧 `reason` 仍然只报 `"dedup"`，不区分"挂起提议覆盖"与"曾被拒绝且情况未变"两种成因；理由是 propose 的 Skipped 本来就是三选一（carry 从不产生 Skipped，只在 `excluded_carry_task_ids` 里悄悄减少候选）的粗粒度信号，为了单独分辨"这次 Skipped 是不是因为拒绝"而给 `ProposeDedup`/`ProposeItemResult` 加一条专门的原因通道，投入和当前 propose 本就模糊的可观测性不成比例；标记为复审触发条件（用户反馈"看不出 propose 到底是被谁挡住的"）。<br><br>**「待定」重新分类**（tasks.md T5.2.2 followup ②，独立机制，完全不查 `sin90_proposal_rejections`）：`AiReadModel::inbox`/`inbox_task`（`store/ai_port.rs`）放宽为 `direction_id IS NULL OR (direction_id = 'sin90-triage' AND updated_at < (SELECT MAX(created_at) FROM sin90_directions WHERE 非终态 AND id != 'sin90-triage'))`——直接把"只在新建 Direction 后重试"这条 T5.2.2 评审给出的判据写进 SQL 的 `WHERE`，而不是先把"待定"任务无条件选进候选、再在 Rust 里二次过滤：`sin90-triage` 是固定 id，子查询与 `direction_candidates` 共享同一个"非终态 + 排除 triage 自己"口径，成本是一次标量子查询，不需要新的 store 方法。`Sin90Op::AssignTaskDirection` 的 A3（`core/proposal.rs`）同步放宽：当前 direction 是 `sin90-triage` 且目标 direction **不是** `sin90-triage` 本身时视为"仍可分配"，回落到 A4/A5 继续校验；`current_direction` 是任何其它非空值、或者目标本身还是 `sin90-triage`（不放宽"待定→待定"，§2 #30 自己的措辞），一律维持原来的 `NotInInbox` 拒绝。`apply_op` 的 CAS `UPDATE`（防御性的第二次检查）同步放宽 `WHERE` 子句为 `direction_id IS NULL OR direction_id = 'sin90-triage'`。 |
| 32 | T5.7.2 评审第二轮：#31 记录的几处判定被发现与实现假设有出入（H1/H2/M1/M2/M3/M6），此条订正 #31 正文里已经过时的措辞，不重写整段 | **订正如下** | **H1（#31 的"情况变化判定"一节自相矛盾）**：#31 原文说"第一腿必须只在任务此刻仍是 `direction_id IS NULL` 时才采信"，并断言"这就是为什么这条判定天然只对纯 inbox 任务生效、不对已经落到「待定」的任务生效"——这句话本身没错，但它带来一个没被 #31 想到的后果：一个已经在「待定」里、被 classify 提议"待定→D"又被人拒绝的任务，`rejectable` 从未把它选进来过（`t.direction_id.is_none()` 直接把它筛掉），所以它自己的这条拒绝**永远不会被查**——待定重新分类的 SQL 闸门（下面④）一旦因为新 Direction 出现而放行这个任务，classify 会**原样再推荐同一个已经被拒绝的 D**，不受§2 #31 本该提供的压制保护，直到用户第二次拒绝。修法：`http::ai_classify::dedup_targets` 的 `rejectable` 去掉 `t.direction_id.is_none()` 这个范围限制，改成对**每一个**候选目标都查压制（待定重试闸门本身仍是下面④独立的机制，两者不冲突，只是不再互相绝缘）。<br><br>**M1（"任务被修改过"腿不可靠）**：`t.updated_at > rejected_at` 这条比较被两个之前没注意到的写路径污染——`Sin90Op::ReorderTasks` 的 apply 会把**整周所有任务**的 `updated_at` 一起 bump（`repo.rs`），跟"这个任务被谁改过"毫无关系；`AssignTaskDirection` 的 CAS 也一样会 bump 目标任务自己的 `updated_at`，H1 修完之后这条腿不能再假设"我只用于 `direction_id IS NULL` 的任务，所以不可能被污染"。改用 `Sin90Store::task_modified_since(task_id, since)`：查 `sin90_events` 里 `entity = 'task' AND entity_id = ? AND kind != 'direction_assigned' AND at > since` 是否存在——`ReorderTasks` 根本不写 `entity = 'task'` 的事件（它只写一条 `week`/`"reordered"`），`direction_assigned` 被显式排除，天然只剩 `"transitioned"`（人类 `PATCH /tasks/{id}` 或 `CarryOverTask` 的收尾）这类真实变更。<br><br>**M2（"新 Direction"腿的时间基准）**：原文用 `rejected_at`；改成 `sin90_proposal_rejections.proposed_at`（`list_rejected_ops` 一并 `SELECT p.created_at AS proposed_at`）——一条提议被人晾了一阵子才拒绝，中间冒出来的新 Direction 应该已经算"情况变了"，不该因为用的是拒绝时刻而不是提议时刻，被误判成"没变"。classify、propose（carry/reorder/create）都同步改。<br><br>**M3（propose 的 reorder 腿补齐）**：原文"propose：只用'新 Direction'这一条腿，`task 被修改过`这条腿本轮不接"——评审发现 reorder 有单一"目标集合"（当周非终态任务），可以复用 M1 同一个 `task_modified_since` 判据：只要**当周任一**任务的 status 变化或内容更新晚于这条被拒 reorder 的 `proposed_at`，就视为情况已变，解除压制；create/carry 仍维持原判（create 无目标任务，carry 复审触发条件不变）。<br><br>**M6（待定重新分类收窄）**：④的 A3 放宽（"待定→真实 Direction"）原文没有区分"AI 兜底放进待定"和"人类/自动化直接 `POST /proposals` 手动归到待定"——收窄为只对前者放行：`ValidationCtx::task_triage_via_classify(task_id)`（`core/proposal.rs`）经 `sin90_ai_calls JOIN sin90_proposals` 反查该任务唯一一次"进待定"的 `AssignTaskDirection` 是否来自已接受、`task_kind='classify'` 的调用；`AiReadModel::inbox`/`inbox_task` 的 SQL 闸门共享同一个 `EXISTS` 子句。用户手动归到待定的任务，两处都不放行——不会被静默重新分类，也不会占用重试候选的名额。同时补上④闸门自己的退出条件（H2）：闸门原文只比较 `updated_at`，一个待定任务被 classify 重新评估但仍判 `none`/低置信/无结论时不会写任何东西回 `sin90_tasks`，于是只要"最新一个非终态非待定 Direction"不变，这个任务就永远满足闸门条件、永远占用 `MAX_CLASSIFY_TASK_IDS` 的一个名额——新表 `sin90_classify_evals(task_id PK, evaluated_at)`（迁移 0015）记录"classify 上次看过这个任务"，闸门比较基准改成 `COALESCE(evaluated_at, 进入待定那条 direction_assigned 事件的 at)`。<br><br>**已知限制（评审要求明确写下，接受、不修）**：<br>&nbsp;&nbsp;- **秒级精度同秒平局按"没变化"处理**：`now_iso8601()` 是秒级分辨率，`proposed_at`/`evaluated_at`/Direction `created_at` 等时间戳在同一秒内产生时，所有比较都用严格 `>`（不是 `>=`），平局一律判定"没有变化"——保守方向（宁可继续压制/继续待定，不误判成"情况变了"），不是 bug。<br>&nbsp;&nbsp;- **时钟回拨不受保护**：所有判定都是字符串/时间戳直接比较，系统时钟如果被人为或异常回拨，`>`/`<=` 的结果可能与实际先后顺序相反；这套机制不检测、不防御时钟回拨，假设服务器时钟单调不回退。<br>&nbsp;&nbsp;- **"新 Direction"是全局判据，不分区域**：`AiReader::max_eligible_direction_created_at()` 是"整个 workspace 里最新的非终态非待定 Direction"，不区分这个新 Direction 和被压制的任务/建议是否有任何主题相关性——用户在无关领域新建一个 Direction，也会解除所有待压制项的压制（这是 §2 #31 原有设计就接受的粒度，本轮沿用，不是新引入的限制）。<br>&nbsp;&nbsp;- **闸门不感知"重新激活"的 Direction（PR#69 评审，接受、不修）**：`triage_retry_gate_sql` 的比较对象只看 `MAX(created_at)`（非终态、排除待定自身）——一个先前被判终态（如 `archived`/`retired`）后又转回非终态的 Direction，其 `updated_at` 会前进但 `created_at` 不变；如果它不是这批里 `created_at` 最新的一个，闸门不会把这次"重新激活"当成新信号，被压制的待定任务不会因此重新获得重试资格，要等到真的出现一个 `created_at` 更新的全新 Direction 为止。修法方向是把比较对象改成 `MAX(created_at, updated_at)`，或专门跟踪 Direction 的状态转换事件；本轮不做，留作已知限制。 |
...
| `sin90_routines`（M3） | `id PK, area_id FK, direction_id FK, title, kind, cron, tz, target_count, target_minutes, status, created_at, updated_at`；+ `kernel_suspended_at TEXT NULL`（T3.3.2，§2 #29，迁移 `0011_outbox_version_kernel_suspended.sql`）——对账器每轮全量对账整列重建（先清空再按内核 `list` 重新置位），写它不 bump `updated_at`、不写事件 | **新**（+ 加列） |
...
| `sin90_outbox`（T3.3.1） | `status` 值域扩 `pending\|done\|failed`（无 CHECK，代码约束）；+ `failure_kind TEXT NULL`、`last_error TEXT NULL`、`attempts INTEGER NOT NULL DEFAULT 0`、`next_attempt_at TEXT NULL`；+ `version INTEGER NOT NULL DEFAULT 1`（T3.3.2 对账器评审 C1，迁移 `0011_outbox_version_kernel_suspended.sql`）——`upsert_outbox` 每次把新期望状态原地覆盖到同一行时 `version` 自增；`outbox_mark_{done,retry,failed}` 的 `WHERE` 都带 `AND version = ?`，一次内核调用在途时行已经被后续变更 collapse 到新 version 就静默不写（下一轮泵按新 version 重发），防止"v1 的成功回执把 v2 标记成已完成"；+ `other_bucket_attempts INTEGER NOT NULL DEFAULT 0`（T3.3.2 对账器评审第 2 轮 L1，迁移 `0012_outbox_other_bucket_attempts.sql`）——`attempts` 是跨所有可重试原因共享的退避计时计数器，"未分类错误连续失败 N 次转 failed(exhausted)" 若也读 `attempts` 会被穿插的 `rate_limited`/`busy`/`not_ready`/`draining` 重试污染；这一列只由未分类失败分支递增，两列在 `upsert_outbox` 的 collapse 里一起清零；+ `result_ref TEXT NULL`（T4.4.1 评审 L5，迁移 `0013_outbox_result_ref.sql`）——outbox 行落地成功（`outbox_mark_done`）时，可选记录内核侧为这次调用产出的标识（典型场景：后续 stacked 分支要接的 `memory.remember` 这个 kind，内核每次成功 `remember` 都会铸造一个全新的 id，值得留痕）；`scheduler.upsert`/`.delete` 没有对应的"新身份"可记，落地时传 `None`，和这一列存在之前的所有行一样读回 `NULL`。`kind` 的值域（无 CHECK，同样靠 Rust 约束）新增第三种：`memory.remember`（T4.4.1，§2 #24）——`finalize_review` 入队，`dedup_key = "review:<id>"`；不加列、不加迁移，`kind`/`desired` 本来就是自由列 | 加列 |
...
- **候选集**：非终态 Direction，按 `updated_at` 倒序至多 **40** 个 ⚖️（`{direction_id, title, status, area_title}`），**永远排除 §2 #30 的系统保留 Direction「待定」**（`direction_candidates` 的 SQL 显式 `AND d.id != 'sin90-triage'`）——它只应该被 T5.2.2 的兜底路径指派，绝不能被 R1/R2/模型当成一个"真候选"选中。候选为空（新用户一个 Direction 都没有）→ 该条目结束为 `nothing`（不写调用行、不产提议），`ClassifyItem.reason = "undetermined"`（M1，2026-09-26 round 2：候选为空时没有模型跑过，不能算"经过判断确实不合适"，不兜底到待定，见 §2 #30）。
...
- **模型（executive / local）**：
  ...
  - 程序复核（scratch `classify::parse`）：容忍一层 ```` ```json ```` 围栏；`deny_unknown_fields`；`choice` 必须在本次键集内（否则 `bad_output`，且这个检查在置信度检查**之前**——一个造出来的键即使配上低置信度也仍是 `bad_output`，不会被误判成"低置信度"）；`choice == "none"` → 本条**决定性无匹配**（记 `ok = 1`——模型认真地说了「不知道」）。
  - **T5.2.2（design §2 #30，用户拍板 Q4，2026-09-26 round 2 收窄）——兜底到「待定」**：**只有**模型步骤真的跑完一次调用、且以 `medium` 或以上把握明确回答 `choice == "none"`（`Outcome::Produced { value: ClassifyDecision { direction_id: None, low_confidence: false, .. }, .. }`）→ 产出 `AssignTaskDirection(t, 待定)`，`ops = [AssignTaskDirection]`，不提议新建 Direction；`source` 复用模型步自己已经产出的 `ok=1` 调用行（`source_for(engine, served_tier)`），仍然是 `local_brain`/`executive`——这次调用**确实**由模型完成，只是它的答案恰好是"待定"。**以下情形一律不兜底，回到裸 `nothing`**（`ClassifyItem.reason = "undetermined"`，留给下次触发重试）：候选集从一开始就是空的（M1）；整条 ladder 跑完没有任何一步产出决定（`Outcome::Nothing`：R1 undecided 且 R2 no_match，没有模型可用或模型全部降级掉——离线、熔断、standalone 无模型，H3）。`choice == "none"` 但 `confidence: "low"` 同样不兜底，走既有的 `reason = "low_confidence"` 路径（H1：`parse_classify_reply` 的 `"none"` 分支以前无条件写死 `low_confidence: false`，本轮改成套用与真实候选键相同的阈值判断）。人类可以像接受任何其它分类提议一样 accept 一条待定提议——待定是一个普通、非终态、`AssignTaskDirection` 的 A4/A5 天然满足的 Direction,没有专门的校验分支。**T5.2.2 本次范围不含**：把已归入待定的任务在用户新建了更合适的 Direction 之后自动重新分类（§2 #30 的 followup，前提 H2 已修）。
  - **T5.2.3（用户拍板 Q8「没把握就不要提」）**：`confidence` 序数化（`low < medium < high`）后低于 `CLASSIFY_CONFIDENCE_THRESHOLD`（`src/ai/classify.rs` 常量）→ 同样 `nothing`、`ok = 1`（调用行照记，只是不出提议），但 `AiRunItem.reason = "low_confidence"`——与 T5.2.2 的「待定」兜底**互斥**：低置信度必须停在 `nothing`，绝不允许也兜底到待定（那仍然是"提交一条模型没把握的提议"，只是换了个 Direction，违背 Q8 本意）；`choice == "none"`（模型主动说不知道，`reason` 不带 `low_confidence` 标记，直接进 T5.2.2 的待定兜底）与低置信度虽然都源自 `direction_id = None`，但成因、结局都不同（前者 `proposed`，后者 `nothing`）。🟡 占位阈值 = `medium`（即只拒 `low`）：这是 Q8 表格里"评审给的技术输入"的直接后果——小模型自报置信度校准很差，真实阈值应等 T5.5.1（J26）按各档命中率来定，现在只给一个保守占位、不做先验拍板；阈值目前是一个 Rust 常量，没有接进 `sin90_settings`/`/settings/ai`（那需要复制 `ai.executive_enabled` 那一整套读写+路由+测试，在有真实数值可调之前不值得），一旦 J26 出数据、需要用户可调，照抄 `ai.executive_enabled` 的形状接入即可。
...
| # | 测试 | 断言 | 正对照 / 变异 |
|---|---|---|---|
| J27 | `classify_low_confidence_` | 模型选真实候选键但 `confidence: "low"`（低于 `CLASSIFY_CONFIDENCE_THRESHOLD = medium`）→ 该条目 `nothing`；调用行仍是 `ok=1`、`error_kind=NULL`、`proposal_id=NULL`（模型认真给了答案，只是不够自信）；`sin90_proposals` 计数为 0；`ClassifyItem.reason == Some("low_confidence")`。`choice == "none"` 配 `confidence: "high"`（或 `"medium"`）**不**是低置信度（`reason == None`）——两种「没结果」成因不同，不能混标；T5.2.2 之后 `choice == "none"` 在这个（medium+ 置信度）分支落成 `proposed(待定)`，`reason` 仍是 `None`（J28（a）钉住）。`choice == "none"` 配 `confidence: "low"` 则是 J28（d）钉住的第三种情形——`nothing`，`reason == Some("low_confidence")`，H1（2026-09-26 round 2）修之前这里曾错误地硬编码成 `low_confidence: false`。造出来的键（`"d9"`）配低置信度仍是 `bad_output`（检查顺序：先查键是否存在，再查置信度），不会被误判成低置信度。reflex（R1）命中时模型桩用 `PanicModel`（被调用即 panic）——命中即证明置信度阈值只作用于模型步，不影响 reflex | 正对照：同一夹具下 `confidence: "medium"`（阈值本身，含）或 `"high"` → 都出提议、`reason == None`；`confidence: "medium"` 单独钉住阈值是「严格小于」而不是「小于等于」 |

**T5.2.2 classify 兜底到「待定」（design §2 #30，用户拍板 Q4）**

| # | 测试 | 断言 | 正对照 / 变异 |
|---|---|---|---|
| J28 | `classify_fallback_` | (a) `classify_fallback_no_match_assigns_triage_direction`：模型真的跑了一次调用、决定性地选 `"none"`（`confidence: "high"`）→ 该条目 `proposed`，`ops == [AssignTaskDirection(t, "sin90-triage")]`（不是新建 Direction 的提议），accept 后任务离开 inbox，事件与 J11 的真匹配路径完全一致；(b) 正对照 `classify_fallback_real_match_assigns_real_direction`：同一夹具下模型选真实候选键 → 提议指向真实 Direction，不是待定；(c) `classify_fallback_triage_excluded_from_gap_directions`：即使 `alloc` 手动给 `sin90-triage` 分了非零配额、且当周没有它的任务，`gap_directions` 也绝不把它列进缺口；(d，H1）`classify_fallback_none_with_low_confidence_stays_nothing`：`choice=="none"` 但 `confidence:"low"` → `nothing`，`reason=Some("low_confidence")`，零提议；(e，H3）`classify_fallback_ladder_exhausted_stays_undetermined`：两个引擎都不可达（`Outcome::Nothing`）→ `nothing`，`reason=Some("undetermined")`，零提议——`classify_remote_down_local_up`（J16）自己的正对照同步改回这个断言；(f，M1）`classify_fallback_no_candidates_stays_undetermined`：候选集为空 → `nothing`，`reason=Some("undetermined")`，零调用行、零提议；(g，H2）`classify_fallback_title_history_excludes_triage_assignments`：评审原话复现——"walk the dog" 归待定 → 人类 accept → 新建「Dog walking」→ 第二条"Walk the dog"必须让模型重新判断（落到 Dog walking），R1 不能凭第一条的待定历史直接决定。另外 `direction_candidates_excludes_the_triage_direction`（`src/store/ai_port.rs`）钉住候选集本身也排除它 | 变异（均已手动"改坏→变红→复原"）：(a) 删掉 `classify_one` 里 `None`（非低置信度、来自 `Outcome::Produced` 的 `"none"`）分支的 `submit_direction_assignment(...)` → 变红；(c) 删掉 `gap_directions` 里 `a.direction_id == TRIAGE_DIRECTION_ID` 的排除 → 变红；(d) `parse_classify_reply` 的 `"none"` 分支还原成硬编码 `low_confidence: false` → 变红（落成 `Proposed`）；(e) `Outcome::Nothing` 分支还原成兜底到待定 → 变红；(f) `candidates.is_empty()` 分支还原成兜底到待定 → 变红；(g) `title_history` 删掉 `AND d.id != ?`（及其 bind）→ 变红（第二条被 R1 误判回待定）；候选集判据（`direction_candidates` 的 `AND d.id != ?`）删掉同样变红 |
...
| Q4 | 没有合适 Direction 时，classify 要不要提议**新建** Direction（需要解决批内前向引用） | 不要 / 要 | 不要。**T5.2.2 落地更新（2026-09-26 round 2，不再是"任务留在 inbox"，且比初版更窄）**：仍然不新建 Direction；只有「模型真的跑了一次调用、并且以 `medium` 或以上把握明确回答 `none`」才归系统保留的「待定」Direction（`AssignTaskDirection(t, 待定)`）——候选集为空、整条引擎梯耗尽、模型低置信度这三种"没结果"一律回到裸 `nothing`（`reason` 分别是 `undetermined`/`undetermined`/`low_confidence`），不冒充"经过判断"，见 §2 #30、§11.7 J28 |
