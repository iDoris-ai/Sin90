...
### 7.1 要不要在协议层强制"AI 不直写"

今天 `POST /directions`、`PATCH /schedule-blocks/{id}`、`PATCH /tasks/{id}` 是直写通道，AI 与人的区分只靠 `Sin90Proposal.source` 这个自报字段——**机制上没有任何东西拦得住一个 AI 直接打直写路由**。三个选项：
- (a) 维持现状，写清楚这是约定不是保证（本文 M0 的默认）；
- (b) 直写路由要求一个只有壳持有的 header（`X-Sin90-Actor: human`），AI 拿不到；
- (c) 关掉所有直写路由，一切走 Proposal —— 代价是人类 UI 每点一下都要两跳。

倾向 (a) → 观察到真实问题再上 (b)。**(c) 不建议**，它会让交互延迟变成产品缺陷。

### 7.2 M0 要不要等 Agent24 的 T13（`agent24-os-sdk`）

今天没有这个 crate，M0 要自己写 ~200 行握手（NDJSON framing + 版本区间 + `initialize`）。两条路：
- **自己写**：M0 立刻能开工，代价是 T13 落地后要换一次（换的是 `adapter_agent24/` 一层，其余三层不动——这正是 §5.2 分层的价值）；
- **先推 Agent24 做 T13**：M0 延后，但 Sin90 顺便成了 SDK 的第一个真实消费者。

倾向**自己写**，理由是隔离层已经把返工范围钉死在一个目录里。

### 7.3 T11 的依赖顺序

计划表里 `T11 依赖 T9, T10`，而用户已决定 **T10（Cos72）暂停**。需要确认：T11 可以跳过 T10 直接做（本文默认如此），还是 T10 里有 T11 必需的东西。**实读判断：没有**——T10 的作用是"提供一个仓库之外的包"，而 T9 的黑盒测试已经自己生成了一个（Python 写的 `blackbox` 模块），T11 不依赖 Cos72 的任何产出。

### 7.4 五大人生系统要不要随包内置

M6 的 Pack 是数据。问题是：默认随 Sin90 分发一套（用户开箱就有五个 Area），还是空白启动让用户自己建？倾向**空白启动 + 一条 `POST /packs/install` 的示例数据**——预置会让"这是我的 OS"变成"这是别人的 OS"。
